...
class Step(nn.Module):
    ...
    def update(self, inp, opt):
        if opt.first_time:
            opt.state[self.mod.weight]['feedback'] = True
            return self.forward_wo_train(inp)
...
#             big_k = calc_big_k_conv(Q_uu, Q_ux)
        ...
        grad = self.mod.weight.grad - opt.lrddp*(term2)
        opt.update([self.mod.weight], [grad])
...
class AdamDDP(Optimizer):
    ...
    def step(self):
        if self.first_time and self.lrddp != 0:
            self.model(update=True, opt=self)
            self.first_time = False
        list_params, list_grad = [], []
        for group in self.param_groups:
            for p in group['params']:
